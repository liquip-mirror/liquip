...
public class UnixVFS : VFSBase
{
    ...
    public override void Initialize(bool i)
    {
        ...
        RegisterFileSystem(new NTFS.Cosmos.NtfsFileSystemFactory());
    ...
    public void RegisterFileSystem(FileSystemFactory aFileSystemFactory)
    {
    ...
    public override DirectoryEntry CreateFile(string aPath)
    {
        ...
        return xFS.Value.FS.CreateFile(xParentEntry, xFileToCreate);
    }
    ...
    {
        Global.mFileSystemDebugger.SendInternal("-- UnixVFS.CreateDirectory ---");
        ...
        var xFS = GetFileSystemFromPath(xParentDirectory);
        return xFS.Value.FS.CreateDirectory(xParentEntry, xDirectoryToCreate);
    }
    ...
    public override bool DeleteDirectory(DirectoryEntry aPath)
    {
        try
        {
            ...
            var xFS = GetFileSystemFromPath(aPath.mFullPath);
            xFS.Value.FS.DeleteDirectory(aPath);
            return true;
        }
        catch
        {
    ...
    {
        Global.mFileSystemDebugger.SendInternal("-- UnixVFS.GetDirectoryListing --");
        var xFS = GetFileSystemFromPath(aPath);
        var xDirectory = DoGetDirectoryEntry(aPath, xFS.Value.FS);
        return xFS.Value.FS.GetDirectoryListing(xDirectory);
    }
    ...
    public override FileAttributes GetFileAttributes(string aPath)
    {
        /*
            ...
            */
        Global.mFileSystemDebugger.SendInternal($"UnixVFS.GetFileAttributes() for path {aPath}");
    ...
    protected virtual void InitializePartitions()
    {
        ...
        if (mPartitions.Count > 0)
        {
            for (int i = 0; i < mPartitions.Count; i++)
            {
                ...
                Global.mFileSystemDebugger.SendInternal(partition.BlockSize);
                global::System.Console.WriteLine("Block Size: " + partition.BlockSize + " bytes");
                Global.mFileSystemDebugger.SendInternal("Block Count:");
                Global.mFileSystemDebugger.SendInternal(partition.BlockCount);
    ...
    protected virtual void InitializeFileSystems()
    {
        ...
        for (int i = 0; i < mPartitions.Count; i++)
        {
            ...
            {
                if (fs.IsType(partition))
                {
                    Global.mFileSystemDebugger.SendInternal($"Partion {i} has a {fs.Name} filesystem");
                    mFileSystems.Add(fs.Create(partition, xRootPath, xSize));
                    break;
                }
            }
    ...
    {
        Global.mFileSystemDebugger.SendInternal("--- UnixVFS.DoGetDirectoryEntry ---");
    ...
    {
        Global.mFileSystemDebugger.SendInternal("--- UnixVFS.GetVolume ---");
    ...
    public override long GetTotalSize(string aDriveId)
    {
        ...
        return xFS.Value.FS.Size * 1024 * 1024;
    }
    ...
    public override long GetAvailableFreeSpace(string aDriveId)
    {
        ...
        return xFS.Value.FS.AvailableFreeSpace;
    }
    ...
    public override long GetTotalFreeSpace(string aDriveId)
    {
        ...
        return xFS.Value.FS.TotalFreeSpace;
    }
    ...
    {
        Global.mFileSystemDebugger.SendInternal("--- UnixVFS.SetFileSystemLabel ---");
        Global.mFileSystemDebugger.SendInternal($"aDriveId {aDriveId} aLabel {aLabel}");

        var xFS = GetFileSystemFromPath(aDriveId);
        xFS.Value.FS.Label = aLabel;
    }
    ...
    public void Format(string aDriveId, string aDriveFormat, bool aQuick)
    {
        var xFS = GetFileSystemFromPath(aDriveId);

        xFS.Value.FS.Format(aDriveFormat, aQuick);
    }
