...
using Liquip.FileSystems.NTFS.Model.Headers;
...
public abstract class Attribute : ISaveableObject
{
    ...
    public virtual void Save(byte[] buffer, int offset)
    {
        ...
        if (NameLength != 0)
        {
            var stringData = Encoding.Unicode.GetBytes(AttributeName);
    ...
    public static Attribute ParseSingleAttribute(byte[] data, int maxLength, int offset = 0)
    {
        ...
        if (res.NonResidentFlag == ResidentFlag.Resident)
        {
            ...
            var bodyOffset = offset + res.ResidentHeader.ContentOffset;
            var length = offset + res.TotalLength - bodyOffset;
        ...
        else if (res.NonResidentFlag == ResidentFlag.NonResident)
        {
            ...
            var bodyOffset = offset + res.NonResidentHeader.ListOffset;
            var length = res.TotalLength - res.NonResidentHeader.ListOffset;
            ...
            {
                var fragments = res.NonResidentHeader.Fragments;
